...
function renderHistory() {
    ...
    document.querySelectorAll('.download-screenshot-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            downloadScreenshot(screenshot, timestamp);
        });
    });
}
    ...
    if (confirm('Cancel this request?')) {
        chrome.runtime.sendMessage({
            ...
            if (response && response.success) {
                ...
                showStatus('Failed to cancel request', 'error');
            }
        });
    }
}
...
// Download screenshot
function downloadScreenshot(screenshotData, timestamp) {
    try {
        ...
        showStatus('Failed to download screenshot', 'error');
    }
}
